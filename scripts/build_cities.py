...
def main():
    ...
    backfills = []       # (place_ids, show, episode, confidence)
    ...
    for (ck, show), eps in sorted(per_city_show.items()):
        ...
        if kind != 'exact':
            ...
            continue
        # The Layover names every episode after one city, so a Layover place
        # in a city the show covered once is genuinely from that episode. The
        # other three also name episodes after countries and regions, so the
        # same reasoning only gets us to "usually right".
        confidence = 'matched' if show == 'the_layover' else 'inferred'
        backfills.append((cities[ck]['places'], show, ep, confidence))
    ...
    n_backfill = 0
    for place_ids, show, ep, confidence in backfills:
        ...
        out.append(
            ...
            f"episode_source = {sql(confidence)}::episode_source "
    ...
    log.append(f'  by confidence           {dict(by_confidence)}')
    log.append('')
